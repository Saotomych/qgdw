...
rm -f $sourcce/m700env.0 > /dev/null 2>&1
...
rm -f $sourcce/uImage > /dev/null 2>&1
...
rm -f $sourcce/u-boot.bin > /dev/null 2>&1
...
rm -f $sourcce/rootfs.cramfs > /dev/null 2>&1
rm -f $sourcce/rootfs.cramfs.md5 2>&1 > /dev/null

cd $home1
echo "put $home1/update.log dev/$ID/logs/update.log" | sftp $UPDATE@$SFTP 2>&1 > /dev/null
