...
//	flastseason->myhldays = create_next_struct_in_list(&(flastseason->myhldays->l), sizeof(LIST));
	...
	ts_printf(STDOUT_FILENO, TS_DEBUG, "Tarif: new season: id=%d name=%s date=%02d.%02d\n",
...
char *key=0, *par=0;
	...
	ts_printf(STDOUT_FILENO, TS_DEBUG, "Tarif: new special working day: id=%d date=%02d.%02d\n", flastspec->id, flastspec->day, flastspec->mnth);
...
char *key=0, *par=0;
	...
	ts_printf(STDOUT_FILENO, TS_DEBUG, "Tarif: new special holiday: id=%d date=%02d.%02d\n", flastspec->id, flastspec->day, flastspec->mnth);
...
char *key=0, *par=0;
	...
	ts_printf(STDOUT_FILENO, TS_DEBUG, "Tarif: new special highday day: id=%d date=%02d.%02d\n", flastspec->id, flastspec->day, flastspec->mnth);
}
...
char *key=0, *par=0;
	...
	ts_printf(STDOUT_FILENO, TS_DEBUG, "Tarif: new set for season %d - '%s': id=%d time=%02d:%02d idtarif=%d\n",
...
char *key=0, *par=0;
	...
	ts_printf(STDOUT_FILENO, TS_DEBUG, "Tarif: new tarif: id=%d name=%s money=%f\n",
...
struct stat fst;
	...
	if (stat(filename, &fst) == -1){
		ts_printf(STDOUT_FILENO, TS_INFO, "Tarif: Config file not found\n");
	}
	...
	{
		ts_printf(STDOUT_FILENO, TS_INFO, "Tarif: config file is incomplete\n");
		exit(1);
	}
...
tarif *actt;
	...
	while(acts){
		...
		while(actwset){
			...
			ts_printf(STDOUT_FILENO, TS_DEBUG, "Tarif: Workday set id=%d connect to tarif id=%d (0x04%X)\n", actwset->id, actt->id, (int) actwset->mytarif);
		...
		while(acthset){
			...
			ts_printf(STDOUT_FILENO, TS_DEBUG, "Tarif: Holiday set id=%d connect to tarif id=%d (0x04%X)\n", acthset->id, actt->id, (int) acthset->mytarif);
	...
	while(acthset){
		...
		ts_printf(STDOUT_FILENO, TS_DEBUG, "Tarif: Highday set id=%d connect to tarif id=%d (0x04%X)\n", acthset->id, actt->id, (int) acthset->mytarif);
