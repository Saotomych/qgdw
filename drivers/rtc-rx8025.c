...
#define RX8025_MAJOR		252

/* i2c client for default RTC Device */
static struct i2c_client *rx8025_i2c_client = NULL;
...
static int rx8025_ioctl(struct inode *inode, struct file *file,
							unsigned int cmd, unsigned long arg)
{
	if (rx8025_i2c_client == NULL)
		return -ENODEV;

	dev_dbg(&rx8025_i2c_client->dev, "ioctl() was called\n");
    ...
    switch(cmd){
    case RTC_RD_TIME:
		dev_dbg(&rx8025_i2c_client->dev, "reading time...\n");
		ret = rx8025_get_time(&rx8025_i2c_client->dev, &tm);
        if (ret)
            break;
		dev_dbg(&rx8025_i2c_client->dev, "read time success\n");
    ...
    case RTC_SET_TIME:
		dev_dbg(&rx8025_i2c_client->dev, "setting time...\n");
        ...
        ret = rx8025_set_time(&rx8025_i2c_client->dev, &tm);
		if(!ret) dev_dbg(&rx8025_i2c_client->dev, "set time success\n");
        break;

    case RTC_PLL_GET:
		dev_dbg(&rx8025_i2c_client->dev, "reading clock precision adjustment...\n");
		ret = rx8025_get_clock_adjust(&rx8025_i2c_client->dev, &adj);
        if (ret)
            break;
		dev_dbg(&rx8025_i2c_client->dev, "read clock precision adjustment success\n");
        ret = copy_to_user(uarg, &adj, sizeof(adj));
        if (ret)
            ret = -EFAULT;
        break;

    case RTC_PLL_SET:
		dev_dbg(&rx8025_i2c_client->dev, "setting clock precision adjustment...\n");
        ret = copy_from_user(&adj, uarg, sizeof(adj));
        if (ret) {
            ret = -EFAULT;
            break;
        }
		ret = rx8025_set_clock_adjust(&rx8025_i2c_client->dev, adj);
		if(!ret) dev_dbg(&rx8025_i2c_client->dev, "set clock precision adjustment success\n");
        break;
...
#if defined(CONFIG_RTC_INTF_DEV) || defined(CONFIG_RTC_INTF_DEV_MODULE)
	err = register_chrdev(RX8025_MAJOR, dev_name(&rx8025->rtc->dev), &rx8025_rtc_fops);

	if (err < 0) {
		dev_err(&client->dev, "unable to register the char device %s\n", dev_name(&rx8025->rtc->dev));
		goto errout_sysfs;
	} else {
		dev_info(&client->dev, "the char device %s registered\n", dev_name(&rx8025->rtc->dev));
	}
#endif

	rx8025_i2c_client = client;
...
#if defined(CONFIG_RTC_INTF_DEV) || defined(CONFIG_RTC_INTF_DEV_MODULE)
	unregister_chrdev(RX8025_MAJOR, dev_name(&rx8025->rtc->dev));
#endif
	rx8025_sysfs_unregister(&client->dev);
	rtc_device_unregister(rx8025->rtc);
	kfree(rx8025);
